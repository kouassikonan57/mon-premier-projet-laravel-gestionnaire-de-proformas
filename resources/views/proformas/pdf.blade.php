...
<body>
    <div class="header clearfix">
        <img src="{{ public_path($proforma->filiale->logo_path ?? 'images/default-logo.png') }}" alt="Logo" class="logo">
        <div class="document-meta">
            <h2 class="titre-proforma">PROFORMA</h2>
    ...
    <div class="infos-supplementaires">
        <p><strong>RÉFÉRENCE : </strong><span style="display:inline-block; width: 60%; text-align: center;"> Proforma N° {{ $proforma->reference }}</span></p>
        <p class="card-text mb-1"><strong>CLIENT : </strong><span style="display:inline-block; width: 70%; text-align: center;">{{ optional($proforma->client)->name ?? 'Client inconnu' }}</span></p>
    </div>
    ...
    @php
        ...
        $montantTVA = 0;
        $montantTTC = 0;
        $tauxTVA = $proforma->tva_rate / 100; // dynamique si taux TVA dans la proforma
        $remise = $proforma->remise ?? 0; // en montant ou en pourcentage selon usage, ici montant fixe en FCFA
    @endphp
    ...
    <table>
        <thead>
            <tr>
                ...
                <th>Total HT</th>
                <th>TVA ({{ $proforma->tva_rate }}%)</th>
                <th>Total TTC</th>
            </tr>
        </thead>
        <tbody>
            @foreach($proforma->articles as $a)
                @php
    ...
    @php
        // Application de la remise (en montant)
        $montantHTApresRemise = max($montantHT - $remise, 0);
    ...
    <table class="total-table">
        ...
        <tr>
            ...
            <td>- {{ number_format($remise, 2, ',', ' ') }} FCFA</td>
        </tr>
        <tr>
        ...
        <tr>
            <td><strong>TVA ({{ $proforma->tva_rate }}%) après remise :</strong></td>
        ...
        @else
        <tr>
            <td><strong>TVA ({{ $proforma->tva_rate }}%) :</strong></td>
